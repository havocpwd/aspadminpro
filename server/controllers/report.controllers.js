...
exports.getOrdersReports = async (req, res)=>{
    ...
    await Order.aggregate([
        {
            $match : { dateIssued : { $gt: start, $lt: end } , orderType:'SaleOrder'}
        },
        {
            $lookup: {
                from: 'partners',
...
exports.getPurchaseOrdersReports = async (req, res)=>{
    var queryParameters = req.query;
    const start = new Date(queryParameters.start);
    start.setHours(0, 0, 0, 0);
    const end = new Date(queryParameters.end);
    ...
    await Order.aggregate([
        {
            $match : { dateIssued : { $gt: start, $lt: end } , orderType:'PurchaseOrder'}
        },
        {
            $lookup: {
                from: 'partners',
...
exports.getPurchaseOrdersByPartners = async (req,res) => {
    var queryParameters = req.query;
    const start = new Date(queryParameters.start);
    start.setHours(0, 0, 0, 0);
    const end = new Date(queryParameters.end);
